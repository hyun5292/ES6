...
*/
////////////////////////////////////////////////////////////////////////////////
////Understand Functional Programming Terminology(술어)
/*
The FCC Team had a mood swing(기분 변화) and now wants two types of tea: green tea
...
to handle various tea requests. We can modify getTea to accept a function as a
parameter to be able to change the type of tea it prepares. This makes getTea more
...
decide the invocation(기도) of that function. You may have seen them passed to other
methods, for example in filter, the callback function tells JavaScript the criteria(기준)
for how to filter an array.

Functions that can be assinged to a variable, passed into another function, or
returned from another function just like any other normal value, are called first
class functions. In JavaScript, all functions are first class functions.
...
functions which were passed in or returned can be called a lambda(람다?).
*/
////////////////////////////////////////////////////////////////////////////////
/*
Prepare 27 cups of green tea and 13 cups of black tea and store them in tea4GreenTeamFCC
and tea4BlackTeamFCC variables, respectively. Note that the getTea function has
been modified so it now takes a function as the first argument.

Note: The data (the number of cups of tea) is supplied as the last argument. We'll
discuss this more in later lessons.
*/
//Function that returns a string representing a cup of green tea
...
/*
Given a function (representing the tea type) and number of cups needed, the following
function returns an array of strings (each representing a cup of a specific type
of tea).
*/
/*
...
you from sneaky bugs. But before we get there, let's look at an imperative(피할 수 없는)
...
Similarly, an imperative style in programming is one that gives the computer a set
of statements to perform a task.

Often the statements change the state of the program, like updating global variables.
A classic example is writing a for loop that gives exact directions to iterate over
...
the tabs you have opened. Let's try to model this using some simple object-oriented
code.
...
prgram, causing incorrect behaviour. The final list of open tabs, stored in finalTabs.tabs,
should be ['FB', 'Gitter', 'Reddit', 'Twitter', 'Medium', 'new tab', 'Netflix',
'YouTube', 'Vine', 'GMail', 'Work mail', 'Docs', 'freeCodeCamp', 'new tab'] but
...
the splice call in the tabClose() funciton. Unfortunately, splice changes the
original array it is called on, so the second call to it used a modified array,
and gave unexpected results.

This is a small example of much larger pattern - you call a function on a variable,
...
changed the orginal array, and resulted in a bug.
...
Let's try to master this discipline(규율) and not alter any variable or object in
our code.
*/
////////////////////////////////////////////////////////////////////////////////
/*
...
We didn't alter the global varaible value, but the function incrementer would not
work without the global variable fixedValue being there.

Another principle of functional programming is to always declare your dependencies
explicity. This means if a function depends on a variable or object being present,
then pass that variable or obejct directly into the function as an argument.
...
would know what you can or cannot change and you can see where the potential traps
are.
...
Write the incrementer function so it takes an argument, and then returns a result
after increasing the value by one.
*/
/*
var fixedValue = 4;  //The global variable
...
1. Don't alter a variable or object - create new variables and objects and return
   them if need be from a function. Hint: using something like var newArr = arrVar,
   where arrVar is an array will simply create a reference to the existing varaible
   and not a copy. So changing a value in newArr would change the value in arrVar.
2. Declare function parameters - any computation inside a function depends only
...
The add function should add the given bookName to the end of the array passed to
it and return a new array (list). The remove function should remove the given
...
prototype. In this exercise we are looking at Array.prototype.map(), or more
simply map.
...
current element being processed. The second is the index of that element and the
...
rating key with the IMDB rating. The code in the editor currently uses a for loop
to do this, so you should replace the loop functionality with your map expression.
*/
// The global variable
/*
var watchList = [
...
var ratings = watchList.map(watch => ({
  title: watch["Title"],
...
You should not use the built-in map method. The Array instance can be accessed in
